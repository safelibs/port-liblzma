...
if [[ ${#tests[@]} -eq 0 ]]; then
  test_sources=("$safe_dir"/tests/upstream/test_*.c)
else
  test_sources=()
  for test_name in "${tests[@]}"; do
    src="$safe_dir/tests/upstream/${test_name}.c"
    if [[ ! -f "$src" ]]; then
...
for src in "${test_sources[@]}"; do
  ...
  # VLI encoding/decoding is implemented in this phase but the generated
  # config intentionally keeps the broad encoder/decoder feature macros
  # undefined. Override them only for this targeted upstream test.
  if [[ "$test_name" == "test_vli" ]]; then
    extra_cppflags+=(-DHAVE_ENCODERS=1 -DHAVE_DECODERS=1)
  fi
