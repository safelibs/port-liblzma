...
use crate::ffi::types::{lzma_action, lzma_allocator, lzma_check, lzma_ret, lzma_stream, LZMA_BUF_ERROR, LZMA_FORMAT_ERROR, LZMA_GET_CHECK, LZMA_MEMLIMIT_ERROR, LZMA_OK, LZMA_OPTIONS_ERROR, LZMA_PROG_ERROR, LZMA_STREAM_END};
...
const SUPPORTED_FLAGS: u32 = LZMA_CONCATENATED | LZMA_TELL_ANY_CHECK | LZMA_IGNORE_CHECK;
...
enum ParseResult {
    NeedMore { consumed: usize, memusage: u64 },
    GetCheck { consumed: usize, memusage: u64 },
    Done {
        consumed: usize,
        output: Vec<u8>,
        memusage: u64,
...
fn decode_payload(payload: &[u8], dict_size: u32) -> Option<Vec<u8>> {
    let mut reader = lzma_rust2::LzmaReader::new(Cursor::new(payload), u64::MAX, 3, 0, 2, dict_size, None).ok()?;
    ...
    for end in min_total..=input.len() {
        ...
        if version == 1 {
            let member_size = u64::from_le_bytes([
                footer[12], footer[13], footer[14], footer[15], footer[16], footer[17],
                footer[18], footer[19],
            ]);
...
fn parse_lzip(
    ...
    loop {
        ...
        let Some((member_end, decoded)) = find_member_end(remaining, version, dict_size, ignore_check) else {
            if finish
...
unsafe fn lzip_decode(
    ...
    if coder.output_pos < coder.output.len() {
        let ret = copy_output(&coder.output, &mut coder.output_pos, output, out_pos, out_size);
    ...
    match parsed {
        ...
        ParseResult::Error { ret, consumed, memusage } => {
            let consume_now = consumed.saturating_sub(base_len).min(in_size);
            coder.input.extend_from_slice(&current[..consume_now]);
            coder.memusage = memusage;
            coder.first_member = false;
    ...
    fn invalid_uncompressed_sizes_fail_in_parser() {
        for name in ["bad-1-v0-uncomp-size.lz", "bad-1-v1-uncomp-size.lz"] {
            match parse_invalid_member(name) {
                ...
                super::ParseResult::NeedMore { .. } => panic!("unexpected need-more parse result for {name}"),
                super::ParseResult::GetCheck { .. } => panic!("unexpected get-check parse result for {name}"),
